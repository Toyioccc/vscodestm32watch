...
static uint8_t OLED_GetFontWidth(uint8_t FontSize)
{
...
void OLED_ShowString(int16_t X, int16_t Y, char *String, uint8_t FontSize)
{
	...
	while (String[i] != '\0')
	{
		...
		if (CharLength == 1)
		{
			...
			{
				/* Tighten 12x24 spacing for clock-like strings. */
				XOffset += (SingleChar[0] == ':') ? 10 : OLED_GetFontWidth(FontSize);
			}
			else
			{
				XOffset += OLED_GetFontWidth(FontSize);
			}
		}
		else
		{
...
{
	uint8_t i;
	uint8_t FontWidth = OLED_GetFontWidth(FontSize);
	for (i = 0; i < Length; i++)
	{



		OLED_ShowChar(X + i * FontWidth, Y, Number / OLED_Pow(10, Length - i - 1) % 10 + '0', FontSize);
	}
}
...
void OLED_ShowSignedNum(int16_t X, int16_t Y, int32_t Number, uint8_t Length, uint8_t FontSize)
{
	...
	for (i = 0; i < Length; i++)
	{
		...
		OLED_ShowChar(X + (i + 1) * FontWidth, Y, Number1 / OLED_Pow(10, Length - i - 1) % 10 + '0', FontSize);
	}
}
...
void OLED_ShowHexNum(int16_t X, int16_t Y, uint32_t Number, uint8_t Length, uint8_t FontSize)
{
	...
	for (i = 0; i < Length; i++)
	{
		...
		if (SingleNumber < 10)
		{
			...
			OLED_ShowChar(X + i * FontWidth, Y, SingleNumber + '0', FontSize);
		}
		else
		{


			OLED_ShowChar(X + i * FontWidth, Y, SingleNumber - 10 + 'A', FontSize);
		}
	}
}
...
void OLED_ShowBinNum(int16_t X, int16_t Y, uint32_t Number, uint8_t Length, uint8_t FontSize)
{
	...
	for (i = 0; i < Length; i++)
	{
		...
		OLED_ShowChar(X + i * FontWidth, Y, Number / OLED_Pow(2, Length - i - 1) % 2 + '0', FontSize);
	}
}
...
void OLED_ShowFloatNum(int16_t X, int16_t Y, double Number, uint8_t IntLength, uint8_t FraLength, uint8_t FontSize)
{
	...
	OLED_ShowNum(X + OLED_GetFontWidth(FontSize), Y, IntNum, IntLength, FontSize);


	OLED_ShowChar(X + (IntLength + 1) * OLED_GetFontWidth(FontSize), Y, '.', FontSize);


	OLED_ShowNum(X + (IntLength + 2) * OLED_GetFontWidth(FontSize), Y, FraNum, FraLength, FontSize);
}
